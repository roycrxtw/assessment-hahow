...
class HahowApiService {
  ...
  static async doGet({ url, retry = API_RETRY }) {
    ...
    try {
      ...
      if (res.status >= 500) { // 500 系列 http status code 時, 重試請求
        return this.doGet({ url, retry: retry - 1 });
      }
  ...
  static async getProfile(id) {
    ...
    const result = await this.doGet({ url: `https://hahow-recruit.herokuapp.com/heroes/${id}/profile` });
  ...
  static async listHero() {
    const results = await this.doGet({ url: 'https://hahow-recruit.herokuapp.com/heroes' });
  ...
  static async getHero(id) {
    ...
    const result = await this.doGet({ url: `https://hahow-recruit.herokuapp.com/heroes/${id}` });
